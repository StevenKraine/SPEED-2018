/*
 * MotionProfileControllerR.h
 *
 *  Created on: Feb 8, 2018
 *      Author: steven
 */

#ifndef SRC_MOTIONPROFILECONTROLLERR_H_
#define SRC_MOTIONPROFILECONTROLLERR_H_
...
class MotionProfileControllerR {
public:
	/**
	...
	MotionProfileControllerR(TalonSRX & talon) :
		RightDrive(talon), _notifer(&MotionProfileControllerR::PeriodicTask, this) {
		/*
	...
	void control() {
		...
		if (RightDrive.GetControlMode() != ControlMode::MotionProfile) {
			...
			RightDrive.GetMotionProfileStatus(_status);
	...
	void startFilling1(const double profile[][3], int totalCnt) {
			/* create an empty point */
			TrajectoryPoint point;
...
#endif /* SRC_MOTIONPROFILECONTROLLERR_H_ */
