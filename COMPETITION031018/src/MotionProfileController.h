...
#ifndef MOTIONPROFILECONTROLLER_H_
#define MOTIONPROFILECONTROLLER_H_
...
public:
	...
	void PeriodicTask_New(const double profile[][3], int totalCnt) {
		TrajectoryPoint point;
		LeftDrive.ChangeMotionControlFramePeriod(5);

		int maxloops = 1;
		LeftDrive.GetMotionProfileStatus(_status);
		while (TopBufferCount <= totalCnt and _status.topBufferRem>=10  )
		{
			...
			LeftDrive.GetMotionProfileStatus(_status);
					 ...
					 if (TopBufferCount == 1) //Init , Starts at 1
						 {
					...
					/* This is fast since it's just into our TOP buffer */
						...
						double positionRot = profile[TopBufferCount][0];
						double velocityRPM = profile[TopBufferCount][1];
						...
						point.timeDur = GetTrajectoryDuration((int) profile[TopBufferCount][2]);
						point.zeroPos = false;
						if (TopBufferCount == 1)
							point.zeroPos = true; /* set this to true on the first point */

						point.isLastPoint = false;
						if ((TopBufferCount) == totalCnt)
						{
						...
						LeftDrive.GetMotionProfileStatus(_status);
						LeftDrive.ProcessMotionProfileBuffer();
						TopBufferCount = TopBufferCount + 1;
						maxloops = maxloops + 1;
						SmartDashboard::PutNumber("TopBufferCount",TopBufferCount);
		}

				LeftDrive.GetMotionProfileStatus(_status);
				while (_status.btmBufferCnt > 1  )
				{
					LeftDrive.GetMotionProfileStatus(_status);
					LeftDrive.ProcessMotionProfileBuffer();
				}

		if (  _status.activePointValid == 1 and _status.isLast == 0) //_status.isUnderrun == false and
			{
		...
		else if (_status.activePointValid && _status.isLast)
			{
	...
	MotionProfileController(TalonSRX & talon) :
			LeftDrive(talon), _notifer(&MotionProfileController::PeriodicTask, this) {
		/*
	...
	void reset() {
		 ...
		 * sitting in memory.
		 */
		TopBufferCount = 1; //Added for team speed 3-22-18
		LeftDrive.ClearMotionProfileTrajectories();
	...
	void startFilling() {
		...
		startFilling(kMotionProfile1, kMotionProfile1Sz);
	}
...
#endif /* MOTIONPROFILECONTROLLER_H_ */
